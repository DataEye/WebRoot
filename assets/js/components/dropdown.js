define(['jquery', 'ractive', 'tmpl', 'lodash', 'components/menu-clean'], function($, Ractive, tmpl, _, clearMenus) {
	var Dropdown = Ractive.extend({
      ...
      function dataSourceChanged(items) {
        ...
        var selectedIndex = 0
        if (val) {
          selectedIndex = _.findIndex(items, {value: value})
        ...
        // 寻找默认选择项目，如果没有则使用第一项
        var index = _.findIndex(items, {value: value})
      ...
      var listener = self.observe('items', dataSourceChanged, {init: false})
      ...
      // 选中的时候先清除监听器
        ...
        listener = self.observe('items', dataSourceChanged, {init: false})
